...
public class ProdukController {
    ...
    public ResponseEntity<ResponseStatus<ProdukResponse>> deleteData(@PathVariable(value = "id") Long id){
        boolean rtn = false;
        ResponseStatus<ProdukResponse> responseData = new ResponseStatus<>();
        rtn = produkService.deleteProdukById(id);
        if(rtn){
            responseData.setStatus(true);
            responseData.setStatusCode(HttpStatus.OK);
            responseData.getMessage().add("Produk dengan ID : "+id+ ", berhasil di hapus");

        }else{
            String message = "Produk dengan ID : "+id+ "| gagal di hapus";
            responseData.setStatus(false);
            responseData.setStatusCode(HttpStatus.OK);
            responseData.getMessage().add("Produk dengan ID : "+id+ ", gagal di hapus");
